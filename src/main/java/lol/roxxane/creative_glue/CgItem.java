...
public class CgItem extends SuperGlueItem {
	public CgItem(Properties properties) {
		super(properties);
	}

	@OnlyIn(Dist.CLIENT)
	public static void spawn_particles(Level world, BlockPos pos, Direction direction, boolean fullBlock, boolean isCreative) {
		...
		if (isCreative) stack = new ItemStack(Items.PURPLE_DYE);
		...
		for (int i = fullBlock ? 40 : 15; i > 0; i--) {
			...
			if (fullBlock) offset =
				new Vec3(Mth.clamp(offset.x, -.5, .5), Mth.clamp(offset.y, -.5, .5),
